...
class open_access_page():
    def __init__(self, access_page):
        ...
        access_page.ID_Entry.config(width = 25, font = 15)
        access_page.ID_Entry.place(x = 225, y =205)
        
        access_page.Password_Entry = Entry(access_page, show = "*")
        access_page.Password_Entry.config(width = 25, font = 15)
        ...
        def login():
            ...
            if access_page.Password_Entry.get() == data[0][2]:
                if data[0][3] == 1 and data[0][4] == 1:
                    access_page.destroy()
                    window = Tk()    
                    gui1 = Raw_add_to_DB(window)
                    main_window = Tk()
                    gui2 = Visualize(main_window)
                    window2 = Tk()
                    gui3 = Product_add_to_DB(window2)
                    window.mainloop()
        ...
        access_page.login_button = Button(access_page, text="Login", font=(20),command=login)
        access_page.login_button.place(x = 225, y =305)
